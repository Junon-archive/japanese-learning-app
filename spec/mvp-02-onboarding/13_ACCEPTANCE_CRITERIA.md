...
6.  `로그인`을 누른 결과가 로그인됨이면 Study Screen, 401이면 Login, 연결 실패면 그 자리의 안내와
    `다시 시도하기`, 로그인 영역 코드 적재 실패면 버튼 없이 상단바 `로그인`을 다시 누르라는 안내이며 공개 화면은
    계속 쓸 수 있다.
...
8.  상단바가 모든 화면에 있고, 앱 이름을 누르면 선택 홈이다. 공개 화면 오른쪽은 `로그인`, Study
    Screen 오른쪽은 `학습 기록`·`로그아웃`·후리가나 토글, Demo 오른쪽은 후리가나 토글·`로그인`, History
    오른쪽은 `로그아웃`뿐이고 Login 오른쪽은 비어 있다.
