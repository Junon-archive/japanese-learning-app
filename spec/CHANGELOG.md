...
-   **[95] 로그인 확인 403과 실패 화면 상단바** (`03_UI_UX_SPEC.md`의 `화면 문구 표`·`상단바`·`로그인 진입`,
    `10_ERROR_HANDLING.md`, mvp-02 12·13): 로그인 진입 순서의 `fetchMe` 결과가 200·401·"그 밖의 실패"뿐이어서
    403(출처 거부)에도 `다시 시도하기`를 두게 읽혔고, 이는 `출처 거부` 문구 행의 "재시도를 권하지 않는다"와
...
**7부 (Wave 3 구현 중 보완):** MVP-02 Wave 3 구현(후리가나 화면, demo 진행·fixture 생성, 가나 화면)에서 명세에 없던
세부를 구현이 정했고, 그 결정을 Wave 3 보완 결정(2026-09-13)으로 명세에 옮겼다. 규칙을 새로 늘리지 않고 구현된
사실을 적었다. 위 서술 중 아래가 바꾼 부분은 아래가 우선한다.

-   **[108] fixture 스크립트의 분석기 부재** (`10_ERROR_HANDLING.md`의 `후리가나 계산 실패 (MVP-02)`): 분석기 부재 행이
    worker·seed·backfill만 적어 demo fixture 스크립트가 분석기를 적재하지 못할 때의 동작이 없었다. 스크립트도 시작에서
    분석기를 적재하고 실패를 잡지 않는다(배포 결함을 문장별 ruby 실패로 흡수하지 않는다는 같은 절의 이유).
-   **[109] 가나 퀴즈 화면 세부** (`03_UI_UX_SPEC.md`의 `화면 문구 표`의 `가나 학습`, `가나 학습`의 `화면 구성`·`라운드`):
    -   히라가나 탭의 외래어에는 문항이 없으므로 퀴즈 카드를 두지 않는다.
    -   퀴즈 화면 제목은 고른 방식의 이름이다. 진행은 `{현재 문항 번호} / {라운드 문항 수}` 숫자 텍스트이고 진행바가
        없으며, 틀린 문항이 끝에 붙으면 전체 수가 늘어난다. 퀴즈 중에도 `글자 표로 돌아가기`가 있고 라운드를 버린다
        (진도는 응답마다 이미 저장된다).
    -   퀴즈 카드의 `{범위}`는 `히라가나 · 청음` 모양이다. 보고 고르기 피드백의 정답은 로마자만 적는다. 가타카나 탭의
        요음·촉음 범위 설명은 작은 글자를 `ャ·ュ·ョ`, `ッ`로 적는다.
    -   라운드 결과의 한 번 더 풀어 본 목록은 글자 옆에 로마자를 적는다. `이번 라운드에서 한 번 더 나와요.` 토스트는
        보고 읽기에서만 띄운다(보고 고르기는 인라인 피드백에 같은 문장).
-   **[110] 상단바 배치와 후리가나 접근성 이름** (`03_UI_UX_SPEC.md`의 `상단바`, `Translation/Furigana`의 `토글과
    렌더링`): 좁은 폭에서 앱 이름과 버튼이 글자 단위로 꺾이는 경우와 Login 화면에서 상단바가 세로 가운데로 밀리는
    경우의 규칙이 없었다. 상단바는 모든 화면(Login 포함)에서 맨 위에 붙고, 단어 안에서 줄을 바꾸지 않으며, 넘치면
    오른쪽 버튼 묶음이 통째로 두 번째 줄로 내려갈 수 있다. 탭 대상 44px는 유지한다. 또 "문장 `aria-label`에 읽기를
    넣지 않는다"만 있어 토글을 켰을 때 tappable 표현 버튼의 접근성 이름에 읽기가 들어가는 것이 허용인지 없었다.
    허용으로 적었다(읽기 제외는 문장 `aria-label`뿐).
-   **[111] demo 진행·저장 세부** (`03_UI_UX_SPEC.md`의 `Demo`의 `진행 규칙`·`진도 저장`·`완료 화면`·`상단바` 표,
    `spec/04_SECURITY_AND_DATA.md`의 `localStorage 사용 범위`, mvp-02 12·13):
    -   [105]의 "가장 최근에 본 문장"이 다시 보기로 보인 시점을 포함하는지 없었다. **본 새 문장 중 fixture 순번이 가장
        큰 문장**이고, 그 문장이 이미 대기열에 있거나 다시 보기를 마쳤으면 넣지 않으며 더 앞 문장으로 내려가지 않는다.
    -   다시 보기 문장에서 한 자기평가는 앞선 값을 덮어쓴다. probe는 `본 문장 수`가 상수의 배수가 되게 한 새 문장
        화면에만 붙고 한 체크포인트에서 한 번이며, 응답 없이 넘어가면 그 표현은 다음 체크포인트 후보로 남는다. 다시
        보기 중 새로고침은 같은 문장이다. 진도 저장 시점은 자기평가·probe 응답·`다음 문장`이다.
    -   "한 번까지만"과 "한 체크포인트에서 한 번"을 새로고침 뒤에도 지키려면 저장값에 다시 보기를 마친 문장과 probe를
        물은 시점이 있어야 한다. 값 목록에 더하고, `spec/04`에 실제 키(`fixtureId`, `position`, `seen`, `selfReports`,
        `probed`, `queue`, `reviewed`)의 표를 두었다. 형식 검증은 원소를 보기 전에 배열 길이 상한(queue·reviewed는
        문장 수, probed는 item 수)을 먼저 본다. 12·13의 값 목록도 맞췄다.
    -   완료 화면의 상단바가 "Demo는 후리가나 토글·`로그인`"과 "토글은 문장이 있는 화면에만" 중 어느 쪽인지 갈렸다.
        뒤쪽을 우선해 `로그인`만 둔다. `상단바` 표와 12·13의 상단바 항목에 적었다.
-   **[112] fixture 스크립트 세부** (`03_UI_UX_SPEC.md`의 `Demo`의 `fixture`): 문장의 한국어 번역이나 item의
    `difficulty_label`이 없는 등 seed 구조 오류는 제외로 흡수하지 않고 exit 2다. 문장당 tappable 상한이 10 이상이면
    fixture의 `sentence_item_id` 체계(문장 id × 10 + 순번)와 겹치므로 exit 2다. 후리가나 계산 요약은 fixture에 들어간
    문장만 센다. presentation과 설명은 스크립트가 API 응답 스키마로 만들고 고정값(`presentation_role` `new`,
    `review_reason` null, `context_stage` `anchor`, `translation_revealed` false, `probe` null)을 넣는다.

7부에서 `updates/backlog.md` 5절에 열 줄(후리가나 켬/끔 e2e 비교의 self-report·probe 응답 경로 부재, demo 진도 검증 속도
테스트의 200ms 기준, 가나 진도 수가 최대 안전 정수일 때 기록 멈춤, fixture 스크립트 stderr 오류 문구의 seed 값 미이스케이프,
